...
import {
  Users, UserCheck, Calendar, UtensilsCrossed, TrendingUp,
  AlertTriangle, CheckCircle, Download, FileSpreadsheet,
  Copy, Check, Zap,
} from 'lucide-react';
...
export default function AdminDashboard() {
  const { darkMode, company, planConfig, plan } = useApp();
  ...
  async function loadAdminData() {
    ...
    // Leave type stats (this month, company-scoped)
    const leaveAll = await supabase
      .from('leaves')
  ...
  const usagePct      = empLimit === Infinity ? 0 : Math.min(100, Math.round((stats.totalEmp / empLimit) * 100));
  ...
  function getSummaryRows() {
    return [
      ...
      ...attendanceTrend.map(r  => ['Attendance Trend',        r.date,  `Present: ${r.present}, Late: ${r.late}, Absent: ${r.absent}`]),
      ...deptData.map(r         => ['Staff by Department',     r.name,  r.value]),
      ...leaveStats.map(r       => ['Leave Types This Month',  r.name,  r.value]),
      ...mealStats.map(r        => ['Meal Orders Today',       r.name,  r.value]),
      ...recentLeaves.map(r     => [
        'Pending Leave',
        r.profiles?.full_name || 'Unknown',
        `${r.leave_type} leave, ${r.start_date} to ${r.end_date}, ${r.days_count} days`,
      ]),
    ];
  }
  ...
  function downloadSummary(fmt) {
    ...
    downloadRows(rows, `admin-summary-${fileDate}`, fmt);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`${card} flex flex-col gap-3`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold mt-0.5">
                <span className="stat-number">{stats.totalEmp}</span>
                <span className={`text-base font-normal ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                  {' / '}{empLimit === Infinity ? '∞' : empLimit} employees
                </span>
              </p>
            </div>
          ...
          <Link
            to="/admin/billing"
            className="inline-flex items-center gap-1.5 self-start text-xs font-semibold text-red-500 hover:text-red-400 transition"
          >
        ...
        <div className={`${card} flex flex-col gap-3`}>
          ...
          <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            Share this link with your employees to let them log in.
          </p>
          <div className={`flex items-center gap-2 rounded-xl border px-3 py-2
            ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
            <span className="flex-1 text-xs font-mono truncate text-red-500">{loginUrl}</span>
            <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={UserCheck}     label="Present Today"   value={stats.presentToday} subtext={`${Math.round((stats.presentToday / Math.max(stats.totalEmp, 1)) * 100)}% attendance`} color="bg-green-500" />
          ...
          <div className="flex justify-around mt-2">
            {mealStats.map(m => (
              <div key={m.name} className="text-center">
                <p className="text-lg font-bold stat-number">{m.value}</p>
      ...
      <div className={card}>
        ...
        {recentLeaves.length === 0 ? (
          ...
          <div className="space-y-3">
            {recentLeaves.map(l => {
              const coveringPerson = l.covering_person_id
                ? allEmployees.find(e => e.id === l.covering_person_id)
                : null;
              return (
