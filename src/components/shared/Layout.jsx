...
import {
  ...
  LogOut, Sun, Moon, Bell, Menu, X, ChevronRight,
  Building2, Users, BarChart3, ClipboardList, Settings
} from 'lucide-react';
...
const adminItems = [
  ...
  { to: '/admin/leaves', icon: ClipboardList, label: 'Leave Approvals' },
  { to: '/admin/meals', icon: UtensilsCrossed, label: 'Meal Management' },
  { to: '/admin/settings', icon: Settings, label: 'Settings' },
];

export default function Layout({ children }) {
  const { profile, darkMode, setDarkMode, signOut, notifications, isAdmin } = useApp();
  ...
  const inactiveClass = darkMode ? 'text-gray-400 hover:text-white hover:bg-gray-800' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50';
  ...
  return (
    <div className={`min-h-screen flex ${darkMode ? 'bg-gray-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 border-r flex flex-col transform transition-transform duration-300
        ...
        {/* Brand */}
        <div className={`flex items-center justify-between px-5 py-5 border-b ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-red-500 to-red-700 rounded-xl flex items-center justify-center shadow-lg shadow-red-500/30">
              <Building2 size={18} className="text-white" />
            </div>
            <div>
              <div className="font-bold text-base leading-tight">AttendX</div>
              <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>HR System</div>
            </div>
          </div>
        ...
        <div className={`px-4 py-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className={`text-xs truncate ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{profile?.department}</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-1">
          <div className={`text-xs font-semibold uppercase tracking-wider px-3 mb-2 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>Main</div>
          ...
          {isAdmin && (
            <>
              <div className={`text-xs font-semibold uppercase tracking-wider px-3 mt-4 mb-2 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>Admin</div>
        ...
        {/* Bottom controls */}
        <div className={`px-3 py-4 border-t ${darkMode ? 'border-gray-800' : 'border-gray-100'} space-y-1`}>
          <button
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        {/* Top bar */}
        <header className={`sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b backdrop-blur-md ${darkMode ? 'bg-gray-950/90 border-gray-800' : 'bg-white/90 border-gray-200'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {notifOpen && (
                <div className={`absolute right-0 mt-2 w-80 rounded-2xl shadow-xl border overflow-hidden z-50 ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`}>
                  <div className={`px-4 py-3 font-semibold text-sm border-b ${darkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                    Notifications
                  </div>
                  {notifications.length === 0 ? (
                    <div className={`px-4 py-6 text-center text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No new notifications</div>
                  ) : (
                    <div className="max-h-64 overflow-y-auto">
                      {notifications.map(n => (
                        <div key={n.id} className={`px-4 py-3 border-b last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-800 transition ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
